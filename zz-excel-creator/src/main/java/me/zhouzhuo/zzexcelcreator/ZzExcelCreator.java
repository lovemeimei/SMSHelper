...
public class ZzExcelCreator implements ExcelManager {
    ...
    @Override
    public ZzExcelCreator fillContentS(List<MyCell> values, WritableCellFormat format) throws WriteException {

        for (MyCell temps:values){
            checkNullFirst();
            checkNullSecond();
            if (format == null)
                writableSheet.addCell(new Label(temps.getCol(), temps.getRow(), temps.getValue()));
            else
                writableSheet.addCell(new Label(temps.getCol(), temps.getRow(), temps.getValue(), format));
        }
